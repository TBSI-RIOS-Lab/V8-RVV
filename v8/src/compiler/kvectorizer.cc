...
bool SLPTree::CanBePacked(const ZoneVector<Node*>& node_group) {
  ...
  if (NodeProperties::IsVOperation(node_group[0]) ||
...
PackNode* SLPTree::BuildTreeRec(const ZoneVector<Node*>& node_group,
  ...
  if (node0->opcode() == IrOpcode::kProtectedLoad ||
    ...
    if (node0->opcode() == IrOpcode::kProtectedLoad) {
      ...
      if (rep != MachineRepresentation::kSimdv) {
        return nullptr;
      }
    ...
    if (node0->opcode() == IrOpcode::kLoadTransform) {
      ...
      if (params.transformation != LoadTransformation::kSVLoad64Splat &&
          params.transformation != LoadTransformation::kSVLoad64Splat) {
...
//////////////////////////////////////////////////////
bool KVectorizer::DecideVectorize() {
  TRACE("Enter %s\n", __func__);
...
void KVectorizer::SetEffectInput(PackNode* pnode, int index, Node*& input) {
  const ZoneVector<Node*>& nodes = pnode->Nodes();
  ...
  for (size_t i = 0; i < nodes.size(); i++) {
    Node* nodev = nodes[i];
    PackNode* effect = GetPackNode(nodev->InputAt(index));
    if (effect == pnode) continue;
    if (effect)
      pnode->SetOperand(index, effect);
    else
      input = nodev->InputAt(index);
    break;
  }
}

void KVectorizer::SetMemoryOpInputs(base::SmallVector<Node*, 2>& inputs,
...
void KVectorizer::CollectSeeds() {
  for (auto it = graph_->GetSimdStoreNodes().begin();
    ...
    if ((GetMemoryOffsetValue(node) % kSize) != 0) {
      continue;
    }
...
bool KVectorizer::ReduceStoreChains(
...
Node* KVectorizer::VectorizeTree(PackNode* pnode) {
    ...
    case IrOpcode::kLoadTransform: {
      ...
      if (params.transformation == LoadTransformation::kSVLoad64Splat) {
        ...
        SetMemoryOpInputs(inputs, pnode, 2);
                 ...
                 LoadTransformation::kSVLoad64Splat) {
        new_op = mcgraph_->machine()->LoadTransform(
...
void KVectorizer::PrintStores(ZoneMap<Node*, StoreNodeSet>* store_chains) {
  if (!v8_flags.trace_wasm_revectorize) {
    return;
  }
  TRACE("Enter %s\n", __func__);
...
void KVectorizer::DetectCPUFeatures() {
  base::CPU cpu;
  if (cpu.has_avx2()) {
...
bool KVectorizer::TryRevectorize(const char* function) {
  bool success = false;
...
bool KVectorizer::ReduceStoreChain(const ZoneVector<Node*>& Stores) {
